...
我们发现第一个函数只能接受一个不可变的变量，也就是在`b=a;`赋值的时候`a`以`const`形式接收，不允许对`b`做出任何修改。那么(3)处只能老老实实地拷贝`a`里面的数据到`b`。

但是第二个函数是`Video&&`，一个rvalue reference，我们可以把`a`里面的数据直接给`b`，不用拷贝，也不会出现双份内存的问题。
...
- 为什么叫rvalue？
    ...
    - rvalue的要求低一些，可以是常量、函数返回值、或者是另一个lvalue。
